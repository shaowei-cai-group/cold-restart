...
void Internal::reset_scores_queue () {
  int *id = (int *)malloc(sizeof(int) * (max_var + 1));
  for (int i = 1; i <= max_var; i++) id[i] = i;
  ...
  printf("%d max_score: %d, last: %d\n", id[max_var], scores.front(), queue.unassigned);
  free(id);
  
  if (!propagate_out_of_order_units ()) {printf("not clean\n"); return;}

  printf("clean %d -> ", stats.current.redundant);
  mark_satisfied_clauses_as_garbage ();
  protect_reasons ();
  mark_all_redundant_clauses_as_garbage ();
  garbage_collection ();
