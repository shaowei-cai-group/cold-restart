...
void testlast(kissat *solver) {
  int x1 = largest_score_unassigned_variable(solver, &solver->scores);
  printf("last VSIDS %d %.2lf\n", x1, kissat_get_heap_score(&solver->scores, x1));
  x1 = largest_score_unassigned_variable(solver, &solver->scores_chb);
  printf("last CHB %d %.2lf\n", x1, kissat_get_heap_score(&solver->scores_chb, x1));
  printf("last VMTF %d\n", last_enqueued_unassigned_variable(solver));
}
