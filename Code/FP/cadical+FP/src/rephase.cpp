...
void Internal::rephase () {
  ...
  else if(count==2)type=rephase_walk();
  else{
    int p=rand()%1000;
    if((p-=opts.rephasep1)<0)type=rephase_best();
    else if((p-=opts.rephasep2)<0)type=rephase_walk();
    else if((p-=opts.rephasep3)<0){for(auto idx:vars)if((val=phases.min[idx]))phases.saved[idx]=val;}
    else if((p-=opts.rephasep4)<0)type=rephase_random();
    else if((p-=opts.rephasep5)<0)type=rephase_original();
    else if((p-=opts.rephasep6)<0)type=rephase_inverted();
    if(rand()%100<opts.rephasep7)type=rephase_flipping();
  }
  /*if (single && !opts.walk) {
